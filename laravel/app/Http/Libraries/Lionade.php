...
class Lionade {
	...
	public function get_rows($table, $param)
	{
		...
		if ($param['input']['dataSearch']) {
			...
			$select = (!empty($input['dataSearch']['from_date'])) ? $select->where('a.created_at', '>=', $input['dataSearch']['from_date'].' 00:00:00') : $select;
			$select = (!empty($input['dataSearch']['to_date'])) ? $select->where('a.created_at', '<=', $input['dataSearch']['to_date'].' 23:59:59') : $select;
		}
